...
void receiveMessage()
{
  ...
  case RED:
    receiveRED();
    setACK();
    studentNode.sendPayload(controller, ACK, ack);
    break;
  case GREEN:
    receiveGREEN();
    setACK();
  ...
  case PING:
    setSTATUS();
    studentNode.sendPayload(controller, STATUS, status);
    break;
  }
}
...
void setACK()
{
  ack[0] = (char)this_node;
  ack[1] = (char)ACK;
  ack[2] = (char)controller;
  ack[3] = (this_node + ACK + controller);
}
...
void setSTATUS()
{
  ...
  status[0] = (char)this_node;
  status[1] = (char)STATUS;
  status[2] = (char)controller;
  status[3] = (char)information;
  status[4] = (char)(this_node + information + STATUS + controller);
}