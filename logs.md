...
- descrição de cada nó para saber a localização geográfica (FUTURE WORK)
- GXX-IXX - nome a atribuir
- Envio dos dados dos alunos para o nó principal para enviar para um tópico em específico (CampusStudentNode e MainNode)
- Fazer um auto ajuste da velocidade de transmissao com o numero de erros (FUTURE WORK)
- Escrever e testar o terceiro lab guide
...
- Students can now ask for more than one sensor alert to the sensor node
- init() function re-ordered for every node type
- Aumentar o countFailedMessages no envio das mensagens do CampusStudentNode
- o student-node deve usar um endianess diferente das outras boards por isso os readings ficam todos diferentes
- Solved bug in alert handling in SensorNode
- Changed the delay for each alert sent by adding a new field to Alert_Request
